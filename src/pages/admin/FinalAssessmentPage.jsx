...
export default function FinalAssessmentPage() {
  ...
  const fetchAssignments = async () => {
    try {
      const { data } = await api.get('/final-assessments/assignments');
      setAssignments(data.data || data);
    } catch (e) {
      toast.error('Gagal memuat penugasan asesmen akhir');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api.put(`/final-assessments/assignments/${editing.id}`, form);
        toast.success('Penugasan asesmen berhasil diperbarui');
      } else {
        await api.post('/final-assessments/assignments', form);
        toast.success('Penugasan asesmen berhasil ditambahkan');
      }
      setShowModal(false);
      setEditing(null);
      ...
      toast.error(e.response?.data?.message || 'Gagal menyimpan penugasan asesmen');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Hapus penugasan ini? Semua jawaban asesmen mahasiswa terkait akan ikut terhapus.')) return;
    try {
      await api.delete(`/final-assessments/assignments/${id}`);
      toast.success('Penugasan asesmen dihapus');
      fetchAssignments();
    } catch (e) {
      toast.error('Gagal menghapus penugasan asesmen');
    }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 className="page-title">🎯 Penugasan Asesmen Akhir</h1>
        <button className="btn btn-primary" onClick={() => { setEditing(null); setForm({ title: '', description: '', type: 'essay', is_active: true }); setShowModal(true); }}>
          <HiOutlinePlus /> Tambah Asesmen Akhir
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="card-body empty-state">
            ...
            <h3>Belum Ada Penugasan Asesmen Akhir</h3>
            <p style={{ marginTop: '8px' }}>Buat kriteria/tugas asesmen akhir pertama untuk mahasiswa PPL.</p>
          </div>
        </div>
      ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 style={{ fontWeight: 700 }}>{editing ? 'Edit Penugasan Asesmen' : 'Tambah Penugasan Asesmen'}</h3>
              <button className="btn-close" onClick={() => setShowModal(false)}>&times;</button>
            </div>
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                <div className="form-group">
                  <label className="form-label">Judul Asesmen</label>
                  <input className="form-input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required placeholder="Contoh: Laporan Akhir PPL UAD" />
                </div>
                <div className="form-group">
                  <label className="form-label">Deskripsi / Instruksi</label>
                  <textarea className="form-input" rows="3" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Instruksi pengerjaan bagi mahasiswa..." />
                </div>
                <div className="form-group">
