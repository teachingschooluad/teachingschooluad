...
export default function StudentsPage() {
  ...
  const openPasswordModal = (student) => {
    setPasswordUserId(student.id);
    setPasswordUserName(student.name);
    setNewPassword('');
                    ...
                    <tr key={student.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      {isAdmin && (
                        <td style={{ padding: '16px', textAlign: 'center' }}>
                          <button className="btn btn-outline" style={{ padding: '6px 12px', fontSize: '12px', marginRight: '8px' }} onClick={() => openPasswordModal(student)}>Password</button>
      ...
      {showPasswordModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '400px', padding: '24px' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: '16px', fontSize: '14px' }}>
              Mengubah password untuk mahasiswa <strong>{passwordUserName}</strong>.
            </p>
