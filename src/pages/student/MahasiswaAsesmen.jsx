...
export default function MahasiswaAsesmen() {
  ...
  const fetchSummary = async () => {
    setLoading(true);
    try {
      const res = await api.get('/final-assessments/summary');
      if (res.data.success) {
        setSummary(res.data.data);
      }
    } catch (err) {
      console.error(err);
      toast.error('Gagal mengambil data asesmen akhir');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAssignment = (item) => {
    ...
    if (item.submitted && item.assessment) {
      setInputText(item.assessment.content_text || '');
      setInputUrl(item.assessment.content_url || '');
    } else {
      setInputText('');
      setInputUrl('');
      setSelectedFiles([]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedItem.assignment.type === 'essay') {
        if (!inputText.trim()) {
          toast.error('Jawaban essay wajib diisi');
          setSubmitting(false);
          return;
        }
      ...
      const res = await api.post('/final-assessments', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      if (res.data.success) {
        toast.success('Asesmen akhir berhasil dikirim!');
        setSelectedItem(null);
        fetchSummary();
      }
    } catch (err) {
      toast.error(err.response?.data?.message || 'Gagal mengirim asesmen');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="student-page fade-in">
      <h2 className="student-page-title">Asesmen Akhir PPL</h2>
      ...
      <div className="student-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '20px' }}>
        <div style={{ background: '#ffffff', borderRadius: '12px', padding: '12px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Total Asesmen</div>
      ...
      <div className="student-list" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {summary?.details?.length === 0 ? (
          <div className="student-empty" style={{ background: '#fff', padding: '30px', textAlign: 'center', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.05)' }}>
            Belum ada penugasan asesmen akhir aktif dari DKL/Admin.
          </div>
        ) : (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span
                    style={{
                      fontSize: '11px',
                      fontWeight: '700',
                      color: item.submitted ? '#22c55e' : '#f59e0b',
                      background: item.submitted ? 'rgba(34, 197, 94, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                      padding: '4px 8px',
                      borderRadius: '6px'
                    }}
                  >
                    {item.submitted ? (item.assessment?.score !== null ? `Nilai: ${item.assessment.score}` : '✓ Dikirim') : '⏳ Belum'}
                  </span>
                </div>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', margin: 0 }}>Detail Asesmen</h3>
              <button
                onClick={() => setSelectedItem(null)}
            ...
            {selectedItem.submitted ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', borderTop: '1px solid #f1f5f9', paddingTop: '16px' }}>
                <div>
                  ...
                  {selectedItem.assignment.type === 'essay' && (
                    <div style={{ padding: '12px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '13px', whiteSpace: 'pre-wrap', color: 'var(--text-primary)' }}>
                      {selectedItem.assessment?.content_text}
                    </div>
                  )}
                  {selectedItem.assignment.type === 'url' && (
                    <a
                      href={selectedItem.assessment?.content_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={{ color: 'var(--primary)', textDecoration: 'underline', fontSize: '13px', fontWeight: '600' }}
                    >
                      🔗 {selectedItem.assessment?.content_url}
                    </a>
                  )}
                  {selectedItem.assignment.type === 'image' && selectedItem.assessment?.content_images && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
                      {selectedItem.assessment.content_images.map((path, idx) => (
                        <img
                          key={idx}
                ...
                {/* Score & Feedback */}
                {selectedItem.assessment?.score !== null ? (
                  <div style={{ padding: '16px', background: 'rgba(0, 61, 122, 0.05)', borderRadius: '12px', border: '1px solid rgba(0, 61, 122, 0.1)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      ...
                      <span style={{ fontSize: '20px', fontWeight: '800', color: 'var(--primary)' }}>{selectedItem.assessment.score}</span>
                    </div>
                    {selectedItem.assessment.feedback && (
                      <div style={{ borderTop: '1px dashed rgba(0, 61, 122, 0.2)', paddingTop: '8px' }}>
                        ...
                        <p style={{ fontSize: '12px', color: 'var(--text-primary)', margin: 0, fontStyle: 'italic' }}>{selectedItem.assessment.feedback}</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <div style={{ padding: '12px', background: '#fffbeb', borderRadius: '8px', border: '1px solid #fef3c7', color: '#b45309', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span>⏳</span> Menunggu penilaian dari Dosen Koordinator / Pembimbing.
                  </div>
                )}
              </div>
            ) : (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', borderTop: '1px solid #f1f5f9', paddingTop: '16px' }}>
                {selectedItem.assignment.type === 'essay' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '12px', fontWeight: '600', color: '#64748b' }}>Jawaban Esai</label>
                    <textarea
                      rows={5}
                      value={inputText}
                ...
                {selectedItem.assignment.type === 'url' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '12px', fontWeight: '600', color: '#64748b' }}>Tautan URL (Google Drive / YouTube / Cloud)</label>
                    <input
                      type="url"
                      value={inputUrl}
                ...
                {selectedItem.assignment.type === 'image' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '12px', fontWeight: '600', color: '#64748b' }}>Unggah Lampiran Gambar (Max 5)</label>
                    <input
                      type="file"
                      accept="image/*"
                      multiple
                    ...
                    {selectedFiles.length > 0 && (
                      <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px' }}>
                        Terpilih: {selectedFiles.map(f => f.name).join(', ')}
                      </div>
                    )}
                  </div>
                )}
                ...
                >
                  {submitting ? <div className="spinner-sm" /> : '🚀 Kirim Asesmen'}
                </button>
              </form>
            )}
          </div>
